 ...
 * layer expansion with exact rational centre-space rooms. Drill pages retain
 * their source IntBox partition; drill pages remain IntBox-partitioned.
 */
...
namespace
{
std::optional<ROUTER_POINT> nearestInRoomAnyAngleMultilayer(
        const ROOM_TERMINAL& aTerminal, ROUTER_POINT aPoint,
        const PLANAR::SIMPLEX& aRoom )
{
...
std::optional<ROOM_MULTILAYER_PATH> MAZE_SEARCH_ENGINE_ANY_ANGLE::FindMultilayerConnection(
        ...
        bool sourceTraceRooms )
{
    ...
    int nextRoomId = 1;
    for( const auto& layer : layers )
        spaces.push_back( std::make_unique<ROOM_SEARCH_ANY_ANGLE>( layer.bounds, layer.obstacles,
    ...
    const auto appendDrillObstacle = [&]( const SHAPE_TREE_ENTRY& aEntry )
    {
        const auto duplicate = [&]( const SHAPE_TREE_ENTRY& aKnown )
        {
            return aKnown.shape.minX == aEntry.shape.minX
                   ...
                   && aKnown.BoundingSimplex().Contains( aEntry.BoundingSimplex() )
                   && aEntry.BoundingSimplex().Contains( aKnown.BoundingSimplex() )
    ...
    auto roomAt = [&]( std::size_t layer, ROUTER_POINT point ) -> ROOM*
    {
        ...
        for( const auto& item : space.tree.Overlaps( seed ) )
            if( item.isRoom && item.BoundingSimplex().Contains( PLANAR::POINT( point ) ) )
                return space.byId.at( item.objectId );
        auto rooms = space.complete( space.incomplete( {
                SIMPLEX::Box( layers[layer].bounds ), layers[layer].id, seed } ) );

        // ExpansionDrill.calculateExpansionRooms requires exactly one room
        // containing the drill location.  ShapeSearchTree may divide
        ...
        for( ROOM* room : rooms )
        {
            if( room->shape->GetSimplex().Contains( PLANAR::POINT( point ) ) )
            {
                if( containing )
                    return nullptr;
                containing = room;
    ...
    for( START_ITEM_SHAPE& shape : startShapes )
    {
        ...
        std::vector<SIMPLEX> cuts;
        cuts.reserve( layers[shape.layer].obstacles.size() + 1 );
        cuts.push_back( SIMPLEX::Box( layers[shape.layer].bounds ) );
                     ...
                     start.start, start.end, cuts ) )
        {
            const bool covered = std::any_of(
                    ...
                    [&]( const auto& entry )
                    {
                        ...
                        return !room->shape->IsObstacle()
                               && room->shape->GetSimplex().Contains(
                                          PLANAR::POINT( seedPoint ) );
                    } );
            if( covered )
                continue;
            ...
            space.complete( space.incomplete( {
                    SIMPLEX::Box( layers[shape.layer].bounds ),
                    layers[shape.layer].id, seed } ) );
        }
    }

    // Queue source-item doors only after the complete-room topology for all
    // source shapes exists, matching AutorouteEngine.initConnection().
        ...
        for( const auto& [id, room] : space.byId )
        {
            ...
            for( const ROOM_LAYER& targetLayer : layers )
                for( const ROOM_TERMINAL& target : targetLayer.targets )
                    ...
                    {
                        const auto candidate = nearestInRoomAnyAngleMultilayer(
                                start, toward, room->shape->GetSimplex() );
                        if( !candidate )
                            continue;
            ...
            STATE state;
            state.room = room;
            state.layer = shape.layer;
            state.entry = { point, point };
    ...
    while( !open.empty() && step() )
    {
        ...
        if( via.transitionsEnabled && current.kind == KIND::PAGE )
        {
            ...
            for( auto& drill : *drills )
            {
                    ...
                    for( std::size_t i = 0; i < layers.size(); ++i )
                    {
                        ...
                        if( !room )
                        {
                            ...
                            if( autorouterDebugEnabled() && drillRoomFailures == 1 )
                            {
                                ...
                                message << "ROOM_ANY_DRILL_ROOM_MISSING net=" << net
                ...
                if( !drill.valid || drill.rooms[current.layer] != current.room->shape.get() )
                {
                    if( drill.valid )
                    {
                        ...
                        if( !loggedRoomMismatch && autorouterDebugEnabled() )
                        {
                            ...
                            std::ostringstream message;
                            message << "ROOM_ANY_DRILL_ROOM_MISMATCH net=" << net
                                    << " drill=(" << drill.location.x << ','
                ...
                const PLANAR::SIMPLEX drillShape = drill.generalFreeShape.value_or(
                        drill.freeShape.ToSimplex().value_or( PLANAR::SIMPLEX::Empty() ) );

                if( drillShape.Dimension() != 2 )
                {
                    drill.valid = false;
                    continue;
                }
            ...
            if( current.kind == KIND::DRILL_ENTER )
            {
                ...
                for( std::size_t to = 0; to < layers.size(); ++to )
                {
                                        ...
                                        state.room->shape.get() ) )
                    {
                        ...
                        // The transition has already paid for entering this
                        // obstacle room.  Do not charge it again when the
                        // destination-layer room is expanded.
        ...
        if( current.kind == KIND::TARGET || current.kind == KIND::FANOUT_TARGET )
        {
            ...
            auto locateCorridor = [&]()
            {
                ...
                if( !located )
                {
                    ...
                    {
                        std::ostringstream message;
                        message << "ROOM_ANY_DRILL_LOCATOR_REJECTED net=" << net
                                << " chain=" << chain.size()
                ...
                autorouterDecisionLog(
                        ...
                        { { "net", std::to_string( net ) },
                          ...
                          { "source_trace_rooms", sourceTraceRooms ? "true" : "false" },
            ...
            for( std::size_t i = 1; i < chain.size() && valid; ++i )
            {
                ...
                {
                    // A same-layer corridor always terminates at DRILL_ENTER
                    // before the corresponding layer-change state is seen.
                    if( !corridor.empty() )
                    {
                        valid = false;
                        break;
                ...
                corridor.push_back(
                        { before.room->shape->GetSimplex(),
                          after.door
                                  ? std::optional<SIMPLEX>(
                                            after.door->GetSimplexShape() )
                                  : std::nullopt,
                          entry } );
        ...
        // Source occupation is delayed until expandToRoomDoors() produced
        // work, allowing another frontier entry into a small/thin room.
        ...
        if( sourceTraceRooms && currentObstacle && !current.alreadyChecked )
        {
                                        ...
                                        MAZE_ADJUSTMENT aAdjustment )
        {
                ...
                && states[current.parent].layer == current.layer )
            {
                ...
                {
                    const auto gravity =
                            previousState.door->GetSimplexShape().CentreOfGravity();
                    centre = { gravity.first, gravity.second };
                }
                bend = MAZE_LIST_ELEMENT::BendPenalty(
                        centre, from, to, layer.bendCost );
            }
            ...
            {
                if( sourceTraceRooms && currentObstacle )
        ...
        {
            ++targetId;
            const auto targetPoint = nearestInRoomAnyAngleMultilayer(
        ...
        if( via.transitionsEnabled
            ...
            for( auto* page : pages.OverlappingPages(
                         current.room->shape->GetSimplex().BoundingBox().value_or( INT_BOX::Empty() ) ) )
            {
    ...
    {
        std::ostringstream message;
        message << "ROOM_ANY_DRILL_FRONTIER_EXHAUSTED net=" << net
