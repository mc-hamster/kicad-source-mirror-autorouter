...
{
    if( m_dimension < 0 )
        m_dimension = GetOctagonShape().Dimension();
...
PLANAR::INT_OCTAGON EXPANSION_DOOR::GetOctagonShape() const
{
    ...
    return m_firstRoom->GetOctagon().Intersection( m_secondRoom->GetOctagon() );
}
                                                          ...
                                                          std::size_t aMaxSections ) const
{
    ...
    {
        auto inside = []( const PLANAR::INT_OCTAGON& aShape, ROUTER_POINT aPoint )
        {
