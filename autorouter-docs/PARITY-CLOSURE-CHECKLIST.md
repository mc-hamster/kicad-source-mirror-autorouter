...
state, source-equivalent negotiated rip-up during optimizer sub-passes, and expansion-time shove alternatives
...
| **14. Optimization** | **Partial, active serial implementation.** The optimizer removes the source-equivalent fork-expanded `Item.getConnectionItems(NONE)` set transactionally, normalizes surviving item records, and reroutes the resulting boundary components (including multi-arm forks). It alternates bounded maze retries with strict pull-tight candidates, compares incompletes/vias/length lexicographically, preserves real pad groups and branch contacts, and trims redundant via/trace tails. `ReadSortedRouteItems` performs the source's dynamic full-board rescan after every attempt, with the strict monotonic x/y/layer cursor, via-first exact-tie behavior, mutable-via-connected trace suppression and insertion-order tie stability. Source normalized-score potential/actual improvement thresholds, increased-cost-to-normal-cost phase transition, per-trace 0.6 rip-up scaling, and odd/even preferred-direction variation are active. `ViaOptimizer` ports weighted two-trace position candidates plus bounded one-trace fanout/plane movement; the latter preserves the exact contacted conduction-area identity and relocates the synthetic terminal atomically. Need source-equivalent negotiated rip-up/rebatching of unaffected connections during optimizer sub-passes, complete 90/45/any-angle pull-tight, arbitrary via contact mutation/recursion, exact trace-combine/via-removal behavior, and source minimum-cumulative-length accounting. `BatchOptimizerMultiThreaded` still delegates serially and lacks global-optimal/greedy/hybrid scheduling. | Completion and full DRC must never degrade; compare via count, length, candidate scores, time and peak memory. |
