...
## Latest core milestone — contextual item/layer clearance classes
...
| **3. Rules and clearance compensation** | **Partial.** Pair/layer clearances, widths, edge and hole constraints exist. Exact source pad contours are associated with each physical pin-padstack layer, and rectangular pin exit restrictions retain their own layer clearance. Explicit item/net/layer clearance conditions are now resolved against the actual host obstacle and carried as authoritative detached-worker matrix entries, including rules which lower netclass defaults. Need layer-shaped via padstacks, contextual item-specific width rules, and a declared policy for rules dependent on future route position/geometry; those conditions remain protected by private-board KiCad DRC at acceptance rather than search-time pre-resolution. | Per-constraint adapter tests and matching host/reference DRC thresholds; deliberately conflicting and non-default rules. |
