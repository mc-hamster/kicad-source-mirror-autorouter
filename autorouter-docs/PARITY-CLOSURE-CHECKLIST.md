...
and branch contacts, and removes redundant via/trace tails safely. Its
...
tie order; peers at an already-consumed exact x/y/layer key are skipped. A
translated `ViaOptimizer` moves representable two-trace vias according to the
...
state, partial connection removal at forks, and expansion-time shove alternatives
...
endpoints. The rectangular room frontier preserves the complete target
centre-line, intersects its integral lattice with the reached room, and attaches
at the nearest exact point. This is implemented in the source-corresponding
`TargetItemExpansionDoor` file and is shared by single- and multilayer room
search. Diagonal start traces still use endpoint seeds, and arbitrary target
item shapes/rational endpoints remain open.
...
| **2. Mutable item model and contacts** | **Partial.** Physical and separate source-tested endpoint/centre normal-contact graphs now exist. A same-layer/same-style run is one polyline item; exact integer junctions split complete polylines with stable first-piece identity and transaction rollback. `Connection.get()` now uses source reverse-ID contact iteration and chain/fork/terminal traversal, with exact item-set trace length and detour. Retained straight copper can be split virtually without editing host items. Paid obstacle rooms distinguish native occupancy connections from source trace/via items. Need rational/curved contacts, full source split/combine normalization, entry restrictions, fixed-state/item-level partial rip-up/removal chains, and precise contact preservation during every change. Immutable host cluster unions and outward-approximated contacts are not a completed port. | Insert/split/join/remove/shove/rollback contact graphs compared to reference, plus independent KiCad connectivity. |
...
| **5. Rooms and doors** | **Active rectangular subset.** Free/incomplete/complete rooms, paid obstacle rooms, source-specific orthogonal obstacle gaps, sorted touching neighbours, overlap doors, section geometry and neighbour-completion lifecycle run in single-layer and multilayer attempts. Exact integral point/axis/oblique trace target shapes now attach through their intersection with each reached rectangular room rather than a diagonal AABB or endpoint-only approximation. Still need full convex/45 geometry, pad/area/general item-region target doors, diagonal start-shape seeding, thin-room/acute-corner and small-door handling, and nonrectangular drill reachability. | Active path must actually visit rooms/door sections, with normalized reference traces—not a unit helper called only by tests. |
| **6. Maze frontier/backtracking** | **Partial, not full equivalence.** The room/drill slice has door-section and drill-layer state, entry geometry, backtracking, paid obstacle-room transitions, exact integral oblique-trace target attachment, and the reference f/g/door-ID/section queue key (including equal-key suppression). Its target IDs are host-local, and its geometry/target regions are otherwise restricted. Default multilayer work uses this frontier first; unsupported/rejected proposals retain grid/visibility fallback. Need exact small-door, shove-adjustment/requeue, item-chain, general target-region and alternative-padstack state plus normalized end-to-end reference decision streams. | First normalized routing-decision divergence, repeated deterministic checkpoints, and boards with routes unavailable to the old grid. |
...
| **14. Optimization** | **Partial, active serial implementation.** The optimizer removes and reroutes complete native connection records transactionally, alternates bounded maze retries with strict pull-tight candidates, compares incompletes/vias/length lexicographically, preserves real pad groups and external branch contacts, and trims redundant via/trace tails. `ReadSortedRouteItems` now performs the source's dynamic full-board rescan after every attempt, with the strict monotonic x/y/layer cursor, via-first exact-tie behavior, mutable-via-connected trace suppression and insertion-order tie stability. `ViaOptimizer` ports weighted two-trace position candidates plus bounded one-trace fanout/plane movement; the latter preserves the exact contacted conduction-area identity and relocates the synthetic terminal atomically. Need item-level `Connection.get()` removal rather than mapping the selected item back to its whole native record, increased/trace-scaled rip-up phases, score thresholds, complete 90/45/any-angle pull-tight, arbitrary via contact mutation/recursion, and exact via removal behavior. `BatchOptimizerMultiThreaded` still delegates serially and lacks global-optimal/greedy/hybrid scheduling. | Completion and full DRC must never degrade; compare via count, length, candidate scores, time and peak memory. |
