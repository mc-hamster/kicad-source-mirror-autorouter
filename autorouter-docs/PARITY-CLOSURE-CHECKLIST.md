...
bounded enlarge, arithmetic corner gravity and nearest point/border projection
...
separate 960-record pinned Java oracle exercises 512 rectangles, triangles, half-planes, wedges,
...
cases and 192 offset/enlarge/gravity/nearest-point cases. `IntOctagon.toSimplex`
...
|---|---|---|
| **1. Geometric primitives** | **Partial.** Exact rational line intersections, line-array polylines and bounded convex entrance/polyline-cutout semantics are source-tested and used by fixed-obstacle spring-over. `Simplex` now includes source-tested stable direction normalization, redundant half-plane removal, empty/lower-dimensional/unbounded states, exact intersection, border removal, translation, rational corners, containment, bounding boxes, rightmost-corner selection, source-rounded support offset, bounded enlarge, arithmetic gravity, nearest point/border projection and source-ordered general-convex shape cutout. `IntOctagon.toSimplex` preserves all eight source support anchors. The general `PolylineArea` stage applies simplex cutout successively with cancellation and bounded failure. Source-named `IntOctagon` core operations include exact cutout dispatch plus all directional border points and stable nearest-border projections. `ShapeSearchTree45Degree.restrainShape`, 45-degree neighbour ordering/edge removal/free and obstacle gaps, and exact octagonal expansion-door sections are pinned-oracle tested and production-built. Expansion rooms retain exact octagons while preserving their bounding boxes for the broad phase, and ordinary/fanout single-/multilayer mazes consume them. The worker DRC uses the same conservative octagonal rectangle offset as search instead of a false-positive square corner. General `TileShape` APIs, arbitrary-angle convex room search and the remaining degenerate shape operations are absent. KiMath shapes and conservative adapter contours are not yet full upstream `IntBox`/`IntOctagon`/`Simplex`/`TileShape`/`Polyline` semantics. Curved/custom copper and holes must not create false contacts or close routable channels. | Extend the pinned differential corpus to degenerate/negative/large coordinates, tangencies, acute angles, narrow corridors, holes and any-angle cases; compare active room decisions against the source. |
