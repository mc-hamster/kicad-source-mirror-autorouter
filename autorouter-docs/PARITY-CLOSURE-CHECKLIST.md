...
sorter. These classes are built into pcbnew, but the active maze room/door
lifecycle is still rectangular: octagonal door ownership, drill rooms and
...
cover 4,096 segment/room combinations; the active rectangular frontier still
uses the older rectangular overload until the 45-degree lifecycle is connected.
...
|---|---|---|
| **1. Geometric primitives** | **Partial.** Exact rational line intersections, line-array polylines and bounded convex entrance/polyline-cutout semantics are source-tested and used by fixed-obstacle spring-over. Source-named `IntOctagon` core operations, `ShapeSearchTree45Degree.restrainShape`, 45-degree neighbour ordering/edge removal/free and obstacle gaps, and exact octagonal expansion-door sections now pass 8,192 pinned Java records and are production-built. Expansion rooms retain exact octagons while preserving their bounding boxes for the existing broad phase. The active maze lifecycle still uses rectangular rooms. General `TileShape` offsets/cutouts, unbounded/degenerate simplex behavior, octagonal drill propagation and arbitrary convex room search remain absent. KiMath shapes and conservative adapter contours are not yet full upstream `IntBox`/`IntOctagon`/`Simplex`/`TileShape`/`Polyline` semantics. Curved/custom copper and holes must not create false contacts or close routable channels. | Extend the pinned differential corpus to full octagon cutout/projection, degenerate/negative/large coordinates, tangencies, acute angles, narrow corridors, holes, 90/45/any-angle cases; then run the active room topology against source decisions. |
...
| **5. Rooms and doors** | **Active rectangular subset.** Free/incomplete/complete rooms, paid obstacle rooms, source-specific orthogonal obstacle gaps, sorted touching neighbours, overlap doors, section geometry and neighbour-completion lifecycle run in single-layer and multilayer attempts. Room and door values now retain exact octagonal geometry, and octagonal door intersection/corner/gravity/section behavior is source-tested, but that geometry is not yet used by the active frontier. Exact integral point/axis/oblique trace starts and targets can attach through rectangular or octagonal reached rooms; bounded seed generation brackets all eight octagonal support directions without enumerating the segment. The active frontier still calls the rectangular overload. Still need an active full 45-degree lifecycle, finite-width pad/area/general item-region doors, rational endpoints, thin-room/acute-corner and small-door handling, and nonrectangular drill reachability. | Active path must actually visit rooms/door sections, with normalized reference traces—not a unit helper called only by tests. |
