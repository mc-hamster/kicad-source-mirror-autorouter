...
| **5. Rooms and doors** | **Active exact 45-degree subset.** Free/incomplete/complete rooms, paid obstacle rooms, exact neighbour gaps, overlap doors, section geometry and neighbour completion now run on `IntOctagon` shapes in ordinary single- and multilayer attempts. Exact integral point/axis/oblique trace starts and targets attach through reached octagonal rooms; bounded seeds bracket all eight support directions. A rectangular fallback remains for non-general-convex single-layer failures, and fanout still uses the rectangular multilayer frontier. Still need finite-width pad/area/general item-region doors, rational endpoints, complete thin-room/acute-corner locator behavior, exact free-drill convex regions, and removal of the fallbacks after qualification. | Match normalized source room/door/frontier/backtrack decisions on constrained and multilayer fixtures, not only geometry helpers. |
