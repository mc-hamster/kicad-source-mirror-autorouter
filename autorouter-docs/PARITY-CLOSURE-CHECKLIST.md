...
| **6. Maze frontier/backtracking** | **Partial, not full equivalence.** The ordinary room/drill slice now has exact-octagonal per-layer rooms and doors, door-section and drill-layer state, entry geometry, backtracking, paid obstacle-room transitions, exact integral oblique-trace start/target attachment, and the reference f/g/door-ID/section queue key (including equal-key suppression). Pending queue work is bounded independently from retained parent chains, avoiding premature exhaustion at dense doors. Target IDs remain host-local, and fanout/geometry/target regions are otherwise restricted. Unsupported/rejected proposals retain the grid/visibility fallback. Need exact small-door, first-drill ordering, shove-adjustment/requeue, item-chain, general target-region and alternative-padstack state plus normalized end-to-end reference decision streams. | First normalized routing-decision divergence, repeated deterministic checkpoints, and boards with routes unavailable to the old grid. |
| **7. Drill/via expansion** | **Partial.** Source `IntBox` drill pages, exact-octagonal free-drill cutout ordering, exact-octagonal full-stack room lookup, drill-layer expansion, SMD pin-centre substitution, and separate geometry invalidation/maze reset now run. Ordered multiple ViaRule entries are selected by transition containment and exact geometry; complete through/blind/buried/microvia spans and types survive reconstruction. Ordinary routing and fanout use the same octagonal stack frontier, with fanout terminating at the first source-layer exit drill. Need host capture of configured non-through rule entries, per-layer padstack shapes/clearance classes, general-convex (non-45-degree) free-drill regions, thin-room and forced-pad checks, and incremental cross-attempt cache invalidation/reuse. | Multilayer fixtures with inactive layers, asymmetric pads, alternative via rules, blocked intermediate layers and drill spacing. |
