...
sorter. These classes now drive the active single-layer maze frontier. Fixed
...
those exact shapes. The previous rectangular frontier remains only as a
bounded transition fallback on layers without general-convex geometry. The
multilayer drill frontier is still rectangular and must be converted before
...
The activation checkpoint passes 177 native autorouter cases and a production
555-board smoke: 21/21 routed connections, zero host-unconnected items, zero
new KiCad DRC violations, eight vias and 122.777 mm of track. This is a safety
checkpoint, not a quality-parity claim; exact multilayer drills, the full
reference backtracker/locator corner policy, any-angle rooms, and broad-board
...
|---|---|---|
| **1. Geometric primitives** | **Partial.** Exact rational line intersections, line-array polylines and bounded convex entrance/polyline-cutout semantics are source-tested and used by fixed-obstacle spring-over. Source-named `IntOctagon` core operations, `ShapeSearchTree45Degree.restrainShape`, 45-degree neighbour ordering/edge removal/free and obstacle gaps, and exact octagonal expansion-door sections now pass 8,192 pinned Java records and are production-built. Expansion rooms retain exact octagons while preserving their bounding boxes for the broad phase, and the active single-layer maze consumes them. General `TileShape` offsets/cutouts, unbounded/degenerate simplex behavior, octagonal drill propagation and arbitrary-angle convex room search remain absent. KiMath shapes and conservative adapter contours are not yet full upstream `IntBox`/`IntOctagon`/`Simplex`/`TileShape`/`Polyline` semantics. Curved/custom copper and holes must not create false contacts or close routable channels. | Extend the pinned differential corpus to full octagon cutout/projection, degenerate/negative/large coordinates, tangencies, acute angles, narrow corridors, holes, 90/45/any-angle cases; compare active room decisions against the source. |
...
| **4. Spatial search trees** | **Partial, now active.** `MinAreaTree` insertion/removal/traversal plus orthogonal and exact-octagonal `completeShape`, restraint and ignore-object/overlap semantics are ported and differentially tested. The host supplies compensated centre-space shapes and rebuilds them per attempt; the octagonal tree drives single-layer room search. Still need exact compensation-class indexing, arbitrary-angle trees, incremental trace updates and source-equivalent room invalidation/reuse after edits. | Multi-obstacle completion sequences, stable visitation/tie order, insertion/removal invalidation and exact free-space coverage. |
| **5. Rooms and doors** | **Active exact single-layer subset.** Free/incomplete/complete rooms, paid obstacle rooms, exact 45-degree neighbour gaps, overlap doors, section geometry and neighbour completion now run on `IntOctagon` shapes in ordinary single-layer attempts. Exact integral point/axis/oblique trace starts and targets attach through reached octagonal rooms; bounded seeds bracket all eight support directions. A rectangular fallback remains for non-general-convex single-layer failures, and the active multilayer drill frontier is still rectangular. Still need finite-width pad/area/general item-region doors, rational endpoints, complete thin-room/acute-corner locator behavior, octagonal drill reachability, and removal of the fallback after qualification. | Match normalized source room/door/frontier/backtrack decisions on constrained and multilayer fixtures, not only geometry helpers. |
