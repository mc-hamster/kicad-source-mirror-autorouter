...
connections and zero new DRC violations, 13 vias, 107.572 mm track, 2,979
expanded nodes and 94 ms core time. The pinned source uses seven vias: the
remaining quality gap is source `optChangedArea` mutation and real item-set
bookkeeping, not room-search runtime.
...
| **13. Fanout** | **Partial, active.** Source-tested component/pin sorting drives actual bounded passes. Searches use dynamic connected/unconnected item terminals, small-net nearest-target fallback, source pass-scaled per-pin budgets, exact-octagonal room/door/drill expansion, first-drill termination, actual landing relocation, ordered net-plus-board ViaRule alternatives, complete selected padstack span/type, early-pass fanout-via rip-up protection, zero-progress/geometry-repeat/three-pass state-repeat convergence, and successful partial-work retention. The geometric landing preplanner and direct shortcuts are gone; failed colocated control items are removed before ordinary routing. The 555 smoke is host-valid without repair but currently uses 13 vias versus the source's seven. Need source-equivalent per-pin `optChangedArea`, replacement of the synthetic control item by real mutable-item topology, host capture of complete configured board/net ViaRules and layer-local padstack geometry, dense BGA policy and broad corpus proof. | Fine-pitch/BGA/through-hole/mixed fixtures, constrained layers, multiple via rules and via-in-pad enabled/disabled; no dangling vias/stubs. |
