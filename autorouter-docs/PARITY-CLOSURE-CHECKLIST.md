...
foundational API parity; arbitrary-angle room activation and the remaining
general `TileShape` surface remain open.
...
`Simplex` cutouts remain open. Fanout now uses the same exact drill frontier and
...
Target-item attachment now also has an exact octagonal lattice clip: it solves
...
so an oblique existing trace can seed every exact 45-degree room it touches
without enumerating millions of lattice points. Randomized brute-force tests
...
|---|---|---|
| **1. Geometric primitives** | **Partial.** Exact rational line intersections, line-array polylines and bounded convex entrance/polyline-cutout semantics are source-tested and used by fixed-obstacle spring-over. `Line` now includes source-rounded support translation, vector translation, signed distance and approximate/exact projection. `Polyline` includes source-tested exact corner/orientation classification, reverse/combine/split/skip, ranged length and bounds, nearest/integral containment, and complete per-segment offset shapes with dog-ear clipping and degenerate physical-shape simplification. `Simplex` includes source-tested stable direction normalization, redundant half-plane removal, empty/lower-dimensional/unbounded states, exact intersection, border removal, translation, rational corners, containment, bounding boxes, rightmost-corner selection, source-rounded support offset, bounded enlarge, arithmetic gravity, nearest point/border projection, area/circumference/width, exact border/corner classification, touching-side selection, line-side distance, interior-segment tests, source-simplified room sectioning and source-ordered general-convex shape cutout. `IntOctagon.toSimplex` preserves all eight source support anchors. The general `PolylineArea` stage applies simplex cutout successively with cancellation and bounded failure. Source-named `IntOctagon` core operations include exact cutout dispatch plus all directional border points and stable nearest-border projections. `ShapeSearchTree45Degree.restrainShape`, 45-degree neighbour ordering/edge removal/free and obstacle gaps, and exact octagonal expansion-door sections are pinned-oracle tested and production-built. Expansion rooms retain exact octagons while preserving their bounding boxes for the broad phase, and ordinary/fanout single-/multilayer mazes consume them. The worker DRC uses the same conservative octagonal rectangle offset as search instead of a false-positive square corner. Rotation/mirroring, projection-line/shortening and remaining general `LineSegment`/`TileShape` transforms/ray operations, arbitrary-angle convex room search and some degenerate operations are absent. KiMath shapes and conservative adapter contours are not yet full upstream semantics. Curved/custom copper and holes must not create false contacts or close routable channels. | Extend the pinned differential corpus to degenerate/negative/large coordinates, tangencies, acute angles, narrow corridors, holes and any-angle cases; compare active room decisions against the source. |
...
| **4. Spatial search trees** | **Partial, now active.** `MinAreaTree` insertion/removal/traversal plus orthogonal and exact-octagonal `completeShape`, restraint and ignore-object/overlap semantics are ported and differentially tested. The host supplies compensated centre-space shapes and rebuilds them per attempt; the octagonal tree drives ordinary single- and multilayer room search. Still need exact compensation-class indexing, arbitrary-angle trees, incremental trace updates and source-equivalent room invalidation/reuse after edits. | Multi-obstacle completion sequences, stable visitation/tie order, insertion/removal invalidation and exact free-space coverage. |
| **5. Rooms and doors** | **Active exact 45-degree subset.** Free/incomplete/complete rooms, paid obstacle rooms, exact neighbour gaps, overlap doors, section geometry and neighbour completion now run on `IntOctagon` shapes in ordinary and fanout single-/multilayer attempts. Exact integral point/axis/oblique trace starts and targets attach through reached octagonal rooms; bounded seeds bracket all eight support directions. A rectangular fallback remains for non-general-convex single-layer failures. Still need finite-width pad/area/general item-region doors, rational endpoints, complete thin-room/acute-corner locator behavior, and removal of the fallbacks after qualification. | Match normalized source room/door/frontier/backtrack decisions on constrained and multilayer fixtures, not only geometry helpers. |
| **6. Maze frontier/backtracking** | **Partial, not full equivalence.** The ordinary room/drill slice now has exact-octagonal per-layer rooms and doors, door-section and drill-layer state, entry geometry, backtracking, paid obstacle-room transitions, exact integral oblique-trace start/target attachment, transition-time ordered ViaRule selection, and the reference f/g/door-ID/section queue key (including equal-key suppression). The selected complete padstack style is retained in the backtracked edge rather than guessed after search. Pending queue work is bounded independently from retained parent chains, avoiding premature exhaustion at dense doors. Target IDs remain host-local, and fanout/geometry/target regions are otherwise restricted. Unsupported/rejected proposals retain the grid/visibility fallback. Need exact small-door, first-drill ordering, shove-adjustment/requeue, item-chain, general target-region and normalized end-to-end reference decision streams. | First normalized routing-decision divergence, repeated deterministic checkpoints, and boards with routes unavailable to the old grid. |
| **7. Drill/via expansion** | **Partial.** Source `IntBox` drill pages, exact-octagonal free-drill cutout ordering, exact-octagonal room lookup, drill-layer expansion, SMD pin-centre substitution, and separate geometry invalidation/maze reset now run. Drill broad-phase obstacles use the source's maximum applicable via radius on each padstack layer; an unrelated layer no longer turns a blind/buried transition into a through-via preflight. Ordered multiple ViaRule entries are selected by transition containment and exact geometry while queueing the transition, and complete through/blind/buried/microvia spans and types survive reconstruction. Ordinary routing and fanout use the same octagonal stack frontier, with fanout terminating at the first source-layer exit drill. Need host capture of configured non-through rule entries, actual layer-local padstack contours/clearance classes (the snapshot still has one uniform diameter), general-convex (non-45-degree) free-drill regions, thin-room and forced-pad checks, and incremental cross-attempt cache invalidation/reuse. | Multilayer fixtures with inactive layers, asymmetric pads, alternative via rules, blocked intermediate layers and drill spacing. |
...
| **9. Path location** | **Active rectangular and octagonal 45-degree subset.** `FoundConnectionLocator45Degree` reconstructs both corridor forms from room/door entries without searching again. Ordered padstack selection and complete span/type reconstruction are active, including fanout. Oblique trace starts and targets attach at exact integral points inside reached rooms. Still need the full source room-shrink, acute/thin-room correction, general pad/area attachment, rational trace contacts, pin exits and exact partial neckdown reconstruction. The legacy locator and any-angle alias remain unported. | Every reconstructed edge and contact agrees with reference constraints before insertion; adversarial short/acute/narrow corridors. |
...
| **13. Fanout** | **Partial, active.** Source-tested component/pin sorting drives actual bounded passes. Searches use dynamic connected/unconnected item terminals, small-net nearest-target fallback, source pass-scaled per-pin budgets, exact-octagonal room/door/drill expansion, first-drill termination, ordered net-plus-board ViaRule alternatives, complete selected padstack span/type, early-pass fanout-via rip-up protection, zero-progress/geometry-repeat/three-pass state-repeat convergence, and successful partial-work retention. The geometric landing preplanner, direct shortcuts and synthetic ratsnest rewrite are gone. A colocated control only names one immutable search request; after each attempt it is retired and only the real pin plus inserted trace/drill remain in electrical topology. Source-safe redundant tail/via cleanup and the active changed-area fixed point run per pin. The 555 smoke is host-valid without repair but currently uses 12 vias versus the source's seven. Need exact 90/45/any-angle per-pin pull-tight/combine behavior, elimination of the remaining search-control adapter, host capture of complete configured board/net ViaRules and layer-local padstack geometry, dense BGA policy and broad corpus proof. | Fine-pitch/BGA/through-hole/mixed fixtures, constrained layers, multiple via rules and via-in-pad enabled/disabled; no dangling vias/stubs. |
