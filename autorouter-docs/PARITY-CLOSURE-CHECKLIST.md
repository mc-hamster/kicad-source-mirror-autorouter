...
yet complete. Batch scoring still derives bend/length/via statistics from the
native connection representation rather than serialized source `Item`s, and
reference thread/time-limit orchestration remains open. Arbitrary
convex/curved item shove, layer-specific padstack
