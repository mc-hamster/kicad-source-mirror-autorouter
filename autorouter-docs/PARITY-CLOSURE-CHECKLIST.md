...
| **11. Shove, spring-over, neckdown** | **Partial, active bounded subset.** Recursive two-direction fixed and movable straight-trace spring-over runs during checked insertion. Circular vias with supported trace/area contacts, including non-uniform layer-shaped padstacks, can move atomically; bridges preserve old-centre contacts. The fixed, unrestricted-angle and retained orthogonal single- and multilayer mazes perform expansion-time same-side shove feasibility against a rolled-back worker transaction, retain partial usable shove length, and queue proven doors before paid rip-up. Pin-entry neckdown now ports `checkTraceSegment`'s Euclidean usable-prefix contract, source one- plus two-coordinate guards, movable-obstacle exclusion, Java rounding and one/two fixed-angle transition corners; only the final pin leg receives the narrow style. Whole-connection neck-width retries remain separate. Need general compensated convex/curved shove, exact source direction-specific mutation, forced pad displacement and arbitrary drill contact graphs. KiCad rule compliance must not be relaxed to imitate reference violations. | Congested fixtures that require displacement rather than reroute alone, exact affected-item logs, and full DRC after each committed operation. |
