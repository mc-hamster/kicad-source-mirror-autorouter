...
| Current native measurement base | `417330e7828ea7ad5d356a59d8491e47e9d629b2` |
...
|---|---|---|
| `autoroute/maze/AutorouteControl.java` | `pcbnew/autorouter/maze/AutorouteControl.h/.cpp` | Source default routing bend cost (zero) is independent from board-history scoring. Nonzero 45/90-degree bend costs use the previous source-door/drill/page centre. Grid-normalized lengths, some per-edge direction costs, UI mapping, and retry scaling remain adapted. |
...
| `autoroute/expansion/ExpansionDoor.java` | `expansion/ExpansionDoor.h/.cpp` | Exact octagonal overlap dimension, line/gravity sections and narrow-door behavior match 2,048 pinned source records and their y-reflected production equivalents. Fixed-direction production executes the corner-order-sensitive operation in source y-up coordinates. The explicit host section-budget guard remains; general-simplex orientation still needs the same production audit. |
