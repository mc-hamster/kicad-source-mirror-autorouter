...
| `board.model.items.Pin`, `core.library.Padstack` trace exits | Layer-local exact pin contour indices, normalized integral exit directions, centre-to-border lengths, package aspect policy, direct pin-to-drill nearest-exit bias, endpoint direction/length checks and strictly inserted `SHOVE_FIXED` rectangular exit stubs | `board/model/items/Pin.h`, `board/KicadBoardAdapter.cpp`, multilayer maze frontiers and `board/optimize/TraceTightener.cpp`; arbitrary offset-shape border reconnection remains open |
