...
| `autoroute/expansion/*.java` | `pcbnew/autorouter/expansion/*.h/.cpp` plus `ExpansionGraph` | Orthogonal neighbours, free-room lifecycle and door sections run in the rectangular slice. Expansion-room values retain exact octagons. `Sorted45DegreeRoomNeighbours` directly implements source octagonal neighbour ordering, edge-removal and free/obstacle incomplete-room gap geometry; `ExpansionDoor` implements exact octagonal intersection, dimension, corner/gravity line and section geometry. `TargetItemExpansionDoor` has exact integral segment clipping and bounded seed generation against all eight support directions. The corresponding octagonal active lifecycle is not connected yet. ExpansionGraph is still the separate legacy visibility graph. |
| `board/searchtree/ShapeSearchTree45Degree.java` | `pcbnew/autorouter/board/searchtree/ShapeSearchTree45Degree.h/.cpp` | Source octagonal leaf filtering, completion, divide-large-room and recursive restraint are production-built; restraint is checked by 2,048 pinned Java records. Neighbour ordering, edge removal and incomplete-room gaps add another 2,048 source records; octagonal doors/drills still block active selection. |
...
| `autoroute/expansion/ExpansionDoor.java` | `expansion/ExpansionDoor.h/.cpp` | Exact octagonal overlap dimension, line/gravity sections and narrow-door behavior match 2,048 pinned source records. The explicit host section-budget guard remains; active search still supplies rectangular rooms. |
