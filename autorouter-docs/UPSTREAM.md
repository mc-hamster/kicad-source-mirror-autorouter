...
| `autoroute/expansion/*.java` | `pcbnew/autorouter/expansion/*.h/.cpp` plus `ExpansionGraph` | Orthogonal neighbours, free-room lifecycle and door sections run in the rectangular slice. `Sorted45DegreeRoomNeighbours` now directly implements source octagonal neighbour ordering, edge-removal and free/obstacle incomplete-room gap geometry instead of aliasing `SortedRoomNeighbours`; octagonal door ownership and the corresponding active lifecycle are not connected yet. ExpansionGraph is still the separate legacy visibility graph. |
