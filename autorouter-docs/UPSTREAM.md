...
search is active for single-layer attempts; the multilayer drill path remains
rectangular, with bounded rectangular and grid/visibility fallbacks. Its scope is documented in the
...
| `autoroute/path/FoundConnectionLocator45Degree.java` | `pcbnew/autorouter/path/FoundConnectionLocator45Degree.h/.cpp` | Active octagonal single-layer corridor locator plus the established rectangular/drill path. Full source shrink/acute/thin-room, pin-exit and multilayer octagonal behavior remain incomplete. |
...
| `autoroute/expansion/*.java` | `pcbnew/autorouter/expansion/*.h/.cpp` plus `ExpansionGraph` | The active single-layer frontier uses exact octagonal free-room completion, neighbour ordering, edge removal, free/obstacle gaps, doors, sections and target attachment. Those primitives are checked by pinned source oracles. Multilayer drill rooms and several finite-width/acute-corner locator semantics remain rectangular or partial. ExpansionGraph is still the separate bounded legacy visibility fallback. |
| `board/searchtree/ShapeSearchTree45Degree.java` | `pcbnew/autorouter/board/searchtree/ShapeSearchTree45Degree.h/.cpp` | Source octagonal leaf filtering, completion, divide-large-room and recursive restraint are production-built, oracle-tested and selected by the active single-layer room frontier. The drill/page frontier still needs exact octagonal propagation. |
...
| `board/searchtree/ShapeSearchTree45Degree.java` | Same relative path, `.h/.cpp` | Exact octagonal broad/narrow tree, completion and restraint. Built, oracle-tested and active for single-layer room search; multilayer drills remain rectangular. |
...
| `autoroute/maze/AutorouteEngine.java`, `MazeSearchEngine.java` | `maze/MazeSearchEngine45Degree.h/.cpp`, `RoomSearchContext45Degree.h`, and existing 90-degree/multilayer files | Active exact-octagonal single-layer lifecycle/frontier with paid obstacle rooms and source-shaped queue state. Rectangular fallback and rectangular through-drill frontier remain transitional. |
