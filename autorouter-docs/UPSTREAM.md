...
search is active for ordinary single-layer and multilayer attempts. Drill pages
and their current free-region decomposition remain rectangular, and fanout
temporarily retains the qualified rectangular multilayer frontier. Bounded
rectangular and grid/visibility fallbacks remain. Its scope is documented in the
...
| `autoroute/maze/MazeSearchEngine.java` | `pcbnew/autorouter/maze/MazeSearchEngine.h/.cpp` | Hybrid boundary: exact-octagonal ordinary single/multilayer room search first, qualified rectangular fanout and transition fallback, then bounded grid/visibility fallback with exact snapshot collision predicates. `MazeSearchEngine45Degree`, `MazeSearchEngine45DegreeMultilayer`, `MazeSearchEngine90Degree`, `MazeSearchEngineMultilayer`, both room contexts, and `MazeSearchEngineRooms` isolate core state and the host adapter. |
...
| `autoroute/pipeline/BatchOptimizer.java` | `pcbnew/autorouter/pipeline/BatchOptimizer.h/.cpp`, `ReadSortedRouteItems.h/.cpp` | Active serial exact-item-chain transaction with fork expansion, dynamic source cursor, source score/cost phases, and whole-board optimizer batch sub-passes. The sub-passes may negotiate an unaffected movable signal connection away and rebatch it on the next pass. Synthetic fanout/plane routes stay on dedicated bounded candidates; complete `optChangedArea`, minimum cumulative length, arbitrary via contacts and parallel scheduling remain open. |
...
| `autoroute/path/FoundConnectionLocator45Degree.java` | `pcbnew/autorouter/path/FoundConnectionLocator45Degree.h/.cpp` | Active octagonal single- and multilayer corridor locator plus the established rectangular fanout path. Full source shrink/acute/thin-room and pin-exit behavior remain incomplete. |
...
| `autoroute/expansion/*.java` | `pcbnew/autorouter/expansion/*.h/.cpp` plus `ExpansionGraph` | Active ordinary single- and multilayer frontiers use exact octagonal free-room completion, neighbour ordering, edge removal, free/obstacle gaps, doors, sections and target attachment. Those primitives are checked by pinned source oracles. Free-drill decomposition and several finite-width/acute-corner locator semantics remain rectangular or partial. ExpansionGraph is still the separate bounded legacy visibility fallback. |
| `board/searchtree/ShapeSearchTree45Degree.java` | `pcbnew/autorouter/board/searchtree/ShapeSearchTree45Degree.h/.cpp` | Source octagonal leaf filtering, completion, divide-large-room and recursive restraint are production-built, oracle-tested and selected by active ordinary single- and multilayer room frontiers. Drill-page free-region decomposition remains rectangular. |
...
| `board/searchtree/ShapeSearchTree45Degree.java` | Same relative path, `.h/.cpp` | Exact octagonal broad/narrow tree, completion and restraint. Built, oracle-tested and active for ordinary single- and multilayer room search; drill-page free regions remain rectangular. |
...
| `autoroute/path/FoundConnectionLocator*.java` | `path/FoundConnectionLocator45Degree.h/.cpp` | Active octagonal and rectangular corridor construction. Full source backtrack state, room shrinking, acute/thin-room correction, pin exits and any-angle location remain open. |
