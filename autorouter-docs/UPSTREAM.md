...
single-layer/multilayer attempts. Bounded rectangular and grid/visibility
fallbacks remain. Its scope is documented in the
...
| `autoroute/maze/MazeSearchEngine.java` | `pcbnew/autorouter/maze/MazeSearchEngine.h/.cpp` | Hybrid boundary: exact-octagonal ordinary and fanout single/multilayer room search first, then bounded grid/visibility fallback with exact snapshot collision predicates. `MazeSearchEngine45Degree`, `MazeSearchEngine45DegreeMultilayer`, `MazeSearchEngine90Degree`, `MazeSearchEngineMultilayer`, both room contexts, and `MazeSearchEngineRooms` isolate core state and the host adapter. |
...
| `autoroute/expansion/*.java` | `pcbnew/autorouter/expansion/*.h/.cpp` plus `ExpansionGraph` | Active ordinary and fanout single- and multilayer frontiers use exact octagonal free-room completion, neighbour ordering, edge removal, free/obstacle gaps, doors, sections, target attachment and exact free-drill decomposition. Those primitives are checked by pinned source oracles. Several finite-width/acute-corner locator semantics remain partial. ExpansionGraph is still the separate bounded legacy visibility fallback. |
